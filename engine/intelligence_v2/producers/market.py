"""
Market dimension producer (Intelligence V2).

Source-agnostic: evaluates market-like Evidence via Phase A aggregation.
Produces market attractiveness (higher score = larger / growing / healthier market).
Does not emit Opportunity Score. Does not mutate Evidence.
Does not consume other DimensionResults.
"""
...
class MarketProducer:
    """DimensionProducer for 'market' (attractiveness)."""
    ...
    def produce(
        ...
        relevant = filter_by_types(list(evidence), _MARKET_TYPES)
        if not relevant:
            return empty_dimension_result(
                self.dimension_name,
                explanation="No market-related evidence available.",
            )

        # positive_is_supporting=True:
        #   POSITIVE → supports higher market attractiveness
        #   NEGATIVE → reduces attractiveness
        ...
        # Source diversity affects confidence / data_quality only, not the score.
        base_conf = (
            0.35 * avg_rel
            + 0.35 * avg_conf
...
def _explain(
    ...
    parts = [
        f"Market attractiveness={score:.2f} from {len(supporting)} supporting "
    ...
    if supporting and contradicting:
        parts.append("Contradictory market signals are present.")
    return " ".join(parts)
